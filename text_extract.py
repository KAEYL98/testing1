...
def get_all_filepaths(root_path:str, restricted_paths:list[str])->list[FilePath]:
    paths = []
...
def read_progress(input_path:str) -> list[str]:
...
def extract_text(pathfile:FilePath, text_root:str, out_root:str) -> None:
...
def write_header_file_ref(out_path:str) -> None:
...
def main():
    ...
    progress = read_progress(PROGRESS_OUT_PATH)
    write_header_file_ref(REF_OUT_PATH)

    paths_list = get_all_filepaths(CWD, [TEXT_PATH, OUT_PATH])
    ...
    for i in tqdm(range(len_paths)):
        ...
        if path.full_path in progress:
            pass
        extract_text(path, TEXT_PATH, OUT_PATH)
        write_one_file_ref(path, REF_OUT_PATH, PROGRESS_OUT_PATH)
